...
> 本文要了解的内容是：
>
> - 了解在Map接口下的子接口HashTable、ConcurrentHashMap中的Hash方法的实现和原理。
> - 为什么JDK1.7和JDK1.8中的hash方法实现的内容不同以及区别？

## 哈希

<font color="red">**Hash，一般翻译做“散列”，也有直接音译为“哈希”的，就是把任意长度的输入，通过散列算法，变换成固定长度的输出，该输出就是散列值。**</font> 这种转换是一种压缩映射，也就是，散列值的空间通常远小于输入的空间，不同的输入可能会散列成相同的输出，所以不可能从散列值来唯一的确定输入值。简单的说就是一种将任意长度的消息压缩到某一固定长度的消息摘要的函数。

所有散列函数都有如下一个基本特性：<font color="red">**根据同一散列函数计算出的散列值如果不同，那么输入值肯定也不同。但是，根据同一散列函数计算出的散列值如果相同，输入值不一定相同。**</font>
...
在 Java 中，保存数据有两种比较简单的数据结构：数据和链表。数组的特点是：寻址容易，插入和删除困难；而链表的特点是：寻址困难，插入和删除容易，上面提到过，常用的哈希函数的冲突解决办法中有一种方法叫链地址法，其实就是将数组和链表组合在一起，发挥两者的优势，(链表的数组)
...
在 JDK 1.7的HashMap其中定义了一个 final int hash(Object k)方法，其主要被以下方法引用：

![](F:\笔记\java_Study\CSDN\三颗心脏大佬\assets\hash方法.jpg)
...
![image-20200619135550407](F:\笔记\java_Study\CSDN\三颗心脏大佬\assets\indexFor中的取模例子.png)
...
以上，就是目前能够得到的结论，但是，由于HashMap使用位运算代替了取模运算，这就带来了另外一个问题，那就是有可能发生冲突。比如：`CA11 0000`和`0001 0000`在对`0000 1111`进行按位与运算后的值是相等的。

![image-20200619135723393](F:\笔记\java_Study\CSDN\三颗心脏大佬\assets\冲突例子.png)
...
这段代码是为了对key的hashCode进行扰动计算，防止不同hashCode的高位不同但低位相同导致的hash冲突。简单点说，就是为了把高位的特征和低位的特征组合起来，降低哈希冲突的概率，也就是说，<font color="red">尽量做到任何一位的变化都能对最终得到的结果产生影响。</font>
...
![image-20200619135858988](F:\笔记\java_Study\CSDN\三颗心脏大佬\assets\冲突解决例子.png)

可以看到，后面的两个hashcode经过位运算之后得到的值也是11 ，虽然我们不知道哪个key的hashcode是上面例子中的那两个，但是肯定存在这样的key，这就产生了冲突。

那么，接下来，我看看一下经过扰动的算法最终的计算结果会如何。

![image-20200619135938341](F:\笔记\java_Study\CSDN\三颗心脏大佬\assets\冲突解决例子2.png)
